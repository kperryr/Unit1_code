...
function filterProducts( arr, func){
...
let upper = products.map((item)=>item.name.toUpperCase());
...
function applyDiscount(percent){
  return function(productPrice){
    return productPrice-(percent*productPrice);
...
products.filter((item)=>item.instock).reduce((total, item) => total + item.price, 0);
